...
for (const member of simpsons) {
  let divElement = document.createElement('div');
    ...
    h3.innerText=`${member.name} ${member.surname} - ${member.age}  років`
    let p=document.createElement('p')
    p.innerText=`${member.info}`
    let img= document.createElement('img')
    img.src=member.photo
    img.alt=member.name
    divElement.append(hr,h3,p,img)
}
...
for (const coursesElement of coursesArray) {
    ...
    divWrap.style.border='2px solid black'
    divWrap.style.width='400px'
    let h2Title=document.createElement('h2')
    h2Title.innerText=`${coursesElement.title}`
    let divInner1=document.createElement('div')
    divInner1.classList.add('displ_flx')
    let divMonth=document.createElement('div')
    divMonth.style.border='2px solid black'
    divMonth.style.marginRight='22px'
    let p1=document.createElement('p')
    p1.innerText=`Тривалість курсу ${coursesElement.monthDuration} місяців`
    divMonth.append(p1)
    let divHour=document.createElement('div')
    divHour.style.border='2px solid black'
    let p2=document.createElement('p')
    p2.innerText=`Тривалість курсу ${coursesElement.hourDuration} годин`
    divHour.append(p2)
    divInner1.append(divMonth,divHour)
    let divInner2=document.createElement('div')
    let ulOpen=document.createElement('ul')
    divInner2.append(ulOpen)
    for (const key in coursesElement.modules) {
        let li=document.createElement('li')
        li.innerText=`${coursesElement.modules[key]}`
        ulOpen.append(li)
    }
    divWrap.append(h2Title,divInner1,divInner2)
